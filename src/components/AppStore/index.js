...
class AppStore extends Component {
  ...
  getFilteredProjects = () => {
    ...
    const filteredProjects = appsList.filter(
      eachAppItem => eachAppItem.category === ActiveId,
    )
  ...
  render() {
    ...
    const searchResults = filteredProjects.filter(eachAppItem =>
      ...
      <div className="background-image">
        ...
        <ul className="items-card">
          {tabsList.map(eachItem => (
            <AppItem
              unique_key={eachItem.id}
              eachTabItem={eachItem}
              clickTabItem={this.clickTabItem}
              isActiveTab={ActiveId === eachItem.tabId}
            />
          ))}
        </ul>
        <ul>
          <div className="app-list">
            {searchResults.map(eachItem => (
              <TabItem unique_key={eachItem.appId} eachAppItem={eachItem} />
            ))}
          </div>
        </ul>
      </div>
